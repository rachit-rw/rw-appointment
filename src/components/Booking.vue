<template>
    <div class="p-4 flex flex-col">
        ...
        <div>
            <input type="text" name="title" placeholder="Appointment is about" v-model="title"
                class="w-full rounded p-2 my-4 bg-gray-200 focus:bg-white border-transparent focus:border-blue-400">
            <v-select class="booking-time mb-4" :options="clients" placeholder="Client" v-model="client" v-if="advisor" />
            
            <v-select class="booking-time" :options="timings" placeholder="Booking time" v-model="timing" />

            <button class="bg-gray-200 focus:bg-white text-gray-700 uppercase tracking-wider text-sm font-bold px-4 py-2 rounded shadow mt-4 w-full"
                @click="bookNow">Book Appointment</button>
        </div>
    </div>
</template>
